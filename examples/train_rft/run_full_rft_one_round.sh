#!/usr/bin/env bash
# run_full_rft_one_round.sh
#
# 单轮 RFT：基于已有的 SFT LoRA，跑 1 轮（采样 → 打分 → 选 top-1 → 训练）。
# 用于：在重新跑完整 3 轮之前，先验证新参数（rep_penalty 等）下采样质量是否过关。
#
# 与 run_full_rft_from_sft.sh 的区别：N_ROUNDS=1。

set -euo pipefail

# =========================================================================
# ⚠️ 必改：已有的 SFT LoRA 路径
# =========================================================================
...
# wandb 实验名
# =========================================================================
RUN_TAG=one_round_$(date +%Y%m%d_%H%M%S)
export WANDB_PROJECT=rc_conv
export WANDB_RUN_GROUP=$RUN_TAG
echo "[run_one_round] WANDB_RUN_GROUP=$WANDB_RUN_GROUP"
...
echo "[run_one_round] SFT_LORA  = $SFT_LORA"
echo "[run_one_round] TMP_ROOT  = $TMP_ROOT"
echo "[run_one_round] LORA_ROOT = $LORA_ROOT"
...
latest_checkpoint() {
    ...
    if [[ -n "$last" && -f "$last/adapter_config.json" ]]; then
        ...
        echo "[run_one_round] ERROR: no adapter_config.json under $d" >&2
        return 1
    fi
}
...
if [[ ! -f "$SFT_LORA/adapter_config.json" ]]; then
    echo "[run_one_round] ERROR: SFT_LORA missing adapter_config.json:" >&2
    echo "  $SFT_LORA" >&2
    exit 1
fi
PREV_LORA=$SFT_LORA
echo "[run_one_round] PREV_LORA = $PREV_LORA"
...
echo
echo "=========================================="
echo "[run_one_round] Round $ROUND"
echo "=========================================="
...
echo
echo "=========================================="
echo "[run_one_round] DONE"
echo "Final LoRA: $PREV_LORA"
echo "Top1 数据落在: $TOP1_JSON"
echo "可手动查看采样质量:"
